...
object TweetLength {
  ...
  def colorForRemainingCharsCount(remaining: Signal[Int]): Signal[String] = {
    Signal[String](if (remaining() >= 15) "green" else if (remaining() <= 14 && remaining() > 0) "orange" else "red")
  }
